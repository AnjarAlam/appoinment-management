...
import { Search, Bell, Moon, Sun, Settings, User, LogOut, Crown } from 'lucide-react';
  ...
  return (
    <div className={`h-16 flex items-center justify-between px-6 border-b sticky top-0 z-50 transition-all duration-300 backdrop-blur-md
      ...
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 bg-gradient-to-br from-purple-600 to-indigo-600 rounded-2xl flex items-center justify-center">
      ...
      <div className="flex-1 max-w-lg mx-8">
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder="Search hospitals, admins, or records..."
            className={`w-full pl-11 pr-4 py-3 rounded-2xl text-sm focus:outline-none transition-all duration-200
              ${isDark 
                ? 'bg-slate-900 border border-slate-700 focus:border-purple-600 text-slate-100 placeholder-slate-500' 
                : 'bg-slate-100 border border-slate-200 focus:border-purple-600 text-slate-900 placeholder-slate-500'
              }`}
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button className={`relative p-3 rounded-2xl transition-all hover:bg-slate-100 dark:hover:bg-slate-800 group`}>
          ...
          <span className="absolute top-2 right-2 w-2 h-2 bg-purple-500 rounded-full ring-2 ring-white dark:ring-slate-950"></span>
        </button>
        ...
        <div className="relative">
          ...
          >
            <div className="w-9 h-9 bg-gradient-to-br from-purple-600 to-indigo-600 rounded-2xl flex items-center justify-center text-white font-semibold shadow-inner">
              SA
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-sm font-medium text-slate-900 dark:text-white">Super Admin</p>
              <p className="text-xs text-slate-500 dark:text-slate-400 -mt-0.5">System Controller</p>
            </div>
          </button>
          ...
          {isUserMenuOpen && (
            <div className={`absolute right-0 mt-3 w-64 rounded-3xl shadow-xl py-2 z-50 border transition-all
              ...
              <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
                <p className="font-medium text-slate-900 dark:text-white">Super Administrator</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">superadmin@clinicnetwork.com</p>
              </div>
