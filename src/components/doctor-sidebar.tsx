...
import { 
  LayoutGrid, Calendar, ClipboardList, FileText,
...
export default function DoctorSidebar() {
  ...
  const menuItems: { label: string; icon: any; href: string; ariaLabel?: string }[] = [
    { label: 'Dashboard', icon: LayoutGrid, href: '/doctor/dashboard', ariaLabel: 'Doctor dashboard' },
    { label: 'Appointments', icon: Calendar, href: '/doctor/appointments', ariaLabel: 'My appointments' },
    { label: 'Diagnosis Reports', icon: FileText, href: '/doctor/diagnosis', ariaLabel: 'Diagnosis & reports' },
    { label: 'Patient Records', icon: ClipboardList, href: '/doctor/patients', ariaLabel: 'Patient records' },
  ];
  ...
  return (
    <div className={`h-screen sticky top-0 left-0 z-50 flex flex-col border-r transition-all duration-300
      ...
      <div className={`p-4 border-b flex items-center ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
        <div className="w-8 h-8 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
          <Stethoscope size={20} />
        </div>
        {!isCollapsed && (
          <div className="ml-3">
            <p className="font-semibold text-lg tracking-tight text-slate-900 dark:text-white">Doctor</p>
            <p className="text-[10px] text-blue-600 font-medium -mt-1">PANEL</p>
          </div>
        )}
      </div>
